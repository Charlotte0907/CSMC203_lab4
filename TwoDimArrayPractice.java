			   		...
			   		if (i==row)
				   		{
					   		for (int z =0; z < intArray[i].length; z++)
					   			{intArray[i][z] = value;
					   			System.out.println("i="+i+ " z="+value);// verification
					   		 animate(row, z);
					   			}
				   		}	
			   	}
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int min=80898523;
	   for (int row=0;row<intArray.length;row++)	   		
		   		//for (int z =0; z < intArray[row].length; z++)
		   			{
		   			if(intArray[row][column] < min)
		   				{min = intArray[row][column];
			   			System.out.println(intArray[row][column]);//verification
			   			System.out.println(min);//verification
		   			}
